...
async def run_backtest(
    ...
    try:
        ...
        backtest_run = models.BacktestRun(
            ...
            config={"initial_capital": initial_capital, "strategy": "Regime-Aware Ensemble"},
        )
...
async def get_backtest_history(current_user: models.User = Depends(auth.get_current_active_user)):
    """Retrieves previous backtest runs for the current user."""
    return (
        await models.BacktestRun.find(models.BacktestRun.user_id == str(current_user.id))
        .sort("-timestamp")
        .to_list()
    )
...
async def get_backtest_result(
    run_id: str, current_user: models.User = Depends(auth.get_current_active_user)
):
