...
async def get_users_overview(current_user: models.User = Depends(auth.get_current_admin)):
    ...
    users = await models.User.find(
        models.User.is_approved == True, models.User.is_superuser == False
    ).to_list()
    ...
    for user in users:
        try:
            ...
            active_trades = await models.Trade.find(
                models.Trade.user_id == str(user.id), models.Trade.status == "OPEN"
            ).to_list()
            ...
            closed_trades = await models.Trade.find(
                models.Trade.user_id == str(user.id), models.Trade.status == "CLOSED"
            ).to_list()
            realized_pnl = sum([getattr(t, "pnl", 0.0) for t in closed_trades])
                ...
                for trade in active_trades:
                    ...
                    try:
                        if len(yf_symbols) > 1:
                            ...
                            # For single series, it might be a DataFrame or Series
                            item = data["Close"] if "Close" in data else data
                            current_price = float(item.iloc[-1])
                        ...
                        logger.error(
                            f"Error processing trade {trade.symbol} for user {user.email}: {e}"
                        )
            except Exception as e:
                logger.error(f"Error downloading yf data for symbols {yf_symbols}: {e}")

            user_portfolio["total_equity"] += user_portfolio["unrealized_pnl"]
            overview_data.append(user_portfolio)
        except Exception as e:
            logger.error(
                f"Critical error in overview for user {user.email if 'user' in locals() else 'unknown'}: {e}"
            )
            continue
