...
# Key: (user_id, symbol)
# Key: (user_id, symbol)
# WeakValueDictionary prevents memory leaks by collecting locks when no longer in use
symbol_locks = WeakValueDictionary()
...
async def get_exchange_symbols(exchange: str):
    ...
    file_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "data", exchange_files[exchange]
    )
...
async def get_prediction(
    ...
    lock = symbol_locks.get(lock_key)
    if not lock:
        lock = asyncio.Lock()
        symbol_locks[lock_key] = lock
    ...
    async with lock:
        try:
            ...
            cutoff = datetime.now(UTC) - timedelta(
                minutes=config.DEDUPLICATION_WINDOW_MINS
            )
            ...
            if not existing_log:
                ...
                logger.info(
                    f"Skipping DB log for {symbol} - duplicate within {config.DEDUPLICATION_WINDOW_MINS}m"
                )
            # ---------------------------
...
async def get_my_history(current_user: models.User = Depends(auth.get_current_active_user)):
    return (
        await models.PredictionLog.find(models.PredictionLog.user_id == str(current_user.id))
        .sort("-timestamp")
        .limit(10)
        .to_list()
    )
...
async def get_history(current_user: models.User = Depends(auth.get_current_admin)):
