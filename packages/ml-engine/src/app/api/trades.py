...
from app.utils.market import get_yf_ticker
...
async def get_active_positions(current_user: models.User = Depends(auth.get_current_active_user)):
    """Returns all open paper trades for the current user, appending current_price."""
    import yfinance as yf

    active_trades = await models.Trade.find(
        models.Trade.user_id == str(current_user.id), models.Trade.status == "OPEN"
    ).to_list()

    if active_trades:
        symbols = list(set([t.symbol for t in active_trades]))
        yf_symbols = [get_yf_ticker(s) for s in symbols]

        try:
            import asyncio

            data_df = await asyncio.to_thread(
                yf.download, yf_symbols, period="1d", interval="1m", progress=False
            )
            data = data_df["Close"]
            for trade in active_trades:
                price_col = get_yf_ticker(trade.symbol)

                try:
                    current_price = (
                        data[price_col].iloc[-1] if len(yf_symbols) > 1 else data.iloc[-1]
                    )
                    trade.current_price = float(current_price)
                except Exception as e:
                    logger.warning(f"Failed to update current price for {trade.symbol}: {e}")
        except Exception as e:
            logger.error(f"Failed to fetch active positions prices: {e}")

    return active_trades


@router.get("/history", response_model=list[models.Trade])
async def get_trade_history(current_user: models.User = Depends(auth.get_current_active_user)):
    """Returns all closed paper trades for the current user."""
    return (
        await models.Trade.find(
            models.Trade.user_id == str(current_user.id), models.Trade.status == "CLOSED"
        )
        .sort("-exit_timestamp")
        .to_list()
    )
...
async def get_performance_snapshot(
    ...
    profit_factor = (
        ...
        if gross_loss > 0
        else (round(gross_profit, 2) if gross_profit > 0 else 0)
    )
    ...
    for t in history_trades:
        ...
        if strat.startswith("HFT"):
            if "Moderate Bullish" in strat:
                strat = "Moderate Bullish"
            elif "High Confidence Bullish" in strat:
                strat = "High Confidence Bullish"
            elif "Moderate Bearish" in strat:
                strat = "Moderate Bearish"
            elif "High Confidence Bearish" in strat:
                strat = "High Confidence Bearish"
    ...
    formatted_strategies = [
        {"name": k, "value": v["count"], "pnl": v["pnl"]} for k, v in strategy_breakdown.items()
    ]
    ...
    sharpe_ratio = (
        round(np.mean(returns) / np.std(returns) * np.sqrt(252), 2)
    ...
    if active_trades:
        ...
        for trade in active_trades:
            ...
            try:
                ...
                logger.warning(f"Unrealized P&L calculation failed for {trade.symbol}: {e}")
                continue
...
async def get_system_config():
    ...
    return {"initial_balance": config.INITIAL_BALANCE, "margin_multiplier": 0.2}


@router.get("/export")
async def export_trades_csv(current_user: models.User = Depends(auth.get_current_active_user)):
    ...
    for t in trades:
        writer.writerow(
            [
                ...
                if t.exit_timestamp
                else t.timestamp.strftime("%Y-%m-%d %H:%M:%S"),
                t.symbol,
                t.strategy,
                t.side,
                t.entry_price,
...
async def close_trade(
    trade_id: str, current_user: models.User = Depends(auth.get_current_active_user)
):
    """Closes an active paper trade."""
    trade = await trading_mgr.close_position(user_id=str(current_user.id), trade_id=trade_id)
    if not trade:
        raise HTTPException(
            status_code=400, detail="Trade closure failed. Position might already be closed."
        )
    return trade
