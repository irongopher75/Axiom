        ...
        try:
            async with httpx.AsyncClient(timeout=15, auth=self._auth) as client:
                ...
                if resp.status_code == 200:
                    ...
                    self._cache = [
                        self._normalize(s) for s in states if s[5] and s[6]
                    ]  # must have lon/lat
                    self._last_update = time.time()
                    logger.info(
                        f"OpenSky: fetched {len(self._cache)} aircraft (Daily calls: {self.daily_calls})"
                    )
...
opensky_service: OpenSkyService | None = None
