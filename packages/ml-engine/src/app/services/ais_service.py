                    ...
                    async for msg in ws:
                        ...
                        if time.time() - self._last_update > 2:
                            ...
                            stale_keys = [
                                ...
                                if now - v.get("last_seen", now) > self.TTL_SECONDS
                            ]
                            for k in stale_keys:
                                del self._cache[k]
                            ...
                            if len(self._cache) > self.MAX_VESSELS:
                                sorted_cache = sorted(
                                    self._cache.items(), key=lambda x: x[1].get("last_seen", 0)
                                )
                            ...
                            await self.ws_manager.broadcast_vessel_data(
                                list(self._cache.values())[:300]
                            )
