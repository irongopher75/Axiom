...
class TradingManager:
    ...
    async def close_position(self, user_id: str, trade_id: str, current_price: float = None):
        ...
        if not trade:
            ...
            any_trade = await models.Trade.find_one(
                models.Trade.id == obj_id, models.Trade.user_id == user_id
            )
            if any_trade:
        ...
        execution = self.executor.route_order(
            trade.symbol, trade.quantity, exit_side, current_price
        )
