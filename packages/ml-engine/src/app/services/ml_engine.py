...
class MarketAnalyzer:
    _CACHE = {}  # Class-level cache: { (symbol, last_ts): processed_df }
    ...
    async def fetch_data(self, period: str = "1mo", interval: str = "1h"):
    ...
    def calculate_indicators(self):
        """Vectorized technical indicator calculations with caching."""
    ...
    def generate_vectorized_signals(self) -> pd.DataFrame:
        ...
        macd_cross_up = (df["MACD"] > df["Signal_Line"]) & (
            df["MACD"].shift(1) <= df["Signal_Line"].shift(1)
        )
        macd_cross_down = (df["MACD"] < df["Signal_Line"]) & (
            df["MACD"].shift(1) >= df["Signal_Line"].shift(1)
        )
