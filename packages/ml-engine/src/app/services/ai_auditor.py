...
class AIAuditor:
    ...
    def verify_prediction(
        self, raw_prediction: dict[str, Any], signal_df: pd.DataFrame
    ) -> dict[str, Any]:
        """
        ...
        if variance > 1.5:  # Signals are pointing in very different directions
            friction_factor = 0.7
            self.log(
                f"High Predictive Friction detected (variance: {round(variance, 2)}). Different indicators are conflicting."
            )
        elif variance < 0.5:
            friction_factor = 1.1
        ...
        if prediction == "BULLISH":
            if regime == MarketRegime.BULL_TREND.value:
                ...
                self.log(
                    "Negative Regime Alignment: Attempting to go Long in a Bear Trend. Raising skepticism."
                )
            ...
            if regime == MarketRegime.BEAR_TREND.value:
                ...
                self.log(
                    "Negative Regime Alignment: Attempting to go Short in a Bull Trend. Raising skepticism."
                )
