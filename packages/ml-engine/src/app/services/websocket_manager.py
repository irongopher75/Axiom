...
from app.services.satellite_service import satellite_ais
...
class WebSocketManager:
    ...
    async def stop(self):
        self._is_running = False
        if self.ais_service:
            await self.ais_service.stop()
        if self.opensky_service:
            await self.opensky_service.stop()
    ...
    async def broadcast_vessel_data(self, current: list):
        """Broadcasts only the changed/new vessels (diff) with satellite augmentation."""
        # Augment with Satellite AIS Intelligence
        current = satellite_ais.augment_vessels(current)
        ...
        for mmsi, v in current_map.items():
            ...
            pos_changed = (
                abs(prev.get("lat", 0) - v.get("lat", 0)) > 0.0001
                or abs(prev.get("lon", 0) - v.get("lon", 0)) > 0.0001
            )
        ...
        removed = [mmsi for mmsi in self.last_vessel_state if mmsi not in current_map]

        if updated or removed:
            message = {
                "type": "VESSEL_DIFF",
                "payload": {"updated": updated, "removed": removed},
                "timestamp": asyncio.get_event_loop().time(),
            }
    ...
    async def broadcast_aircraft_data(self, current: list):
        ...
        for icao, a in current_map.items():
            ...
            pos_changed = (
                abs(prev.get("lat", 0) - a.get("lat", 0)) > 0.001
                or abs(prev.get("lon", 0) - a.get("lon", 0)) > 0.001
            )
        ...
        removed = [icao for icao in self.last_aircraft_state if icao not in current_map]
    ...
    async def subscribe_to_symbol(self, symbol: str):
        ...
        try:
            await self.connections["finnhub"].send(
                json.dumps({"type": "subscribe", "symbol": symbol_to_sub})
            )
