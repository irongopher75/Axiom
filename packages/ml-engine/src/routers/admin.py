...
import common_db.duckdb_client as db, schemas
...
async def get_users_overview(current_user: models.User = Depends(auth.get_current_admin)):
    ...
    for user in users:
        try:
            ...
            try:
                ...
                for trade in active_trades:
                    ...
                    try:
                        ...
                        logger.error(f"Error processing trade {trade.symbol} for user {user.email}: {e}")
            except Exception as e:
            ...
            logger.error(f"Critical error in overview for user {user.email if 'user' in locals() else 'unknown'}: {e}")
            continue
