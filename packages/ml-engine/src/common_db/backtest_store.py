...
class BacktestStore:
    ...
    async def save_run(self, run_id: str, symbol: str, exchange: str, strategy: str, start_ts: str, end_ts: str, config: dict):
        sql = """
            INSERT INTO backtest_runs (id, symbol, exchange, strategy, start_ts, end_ts, config)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """
        await self.db.execute(sql, [run_id, symbol, exchange, strategy, start_ts, end_ts, json.dumps(config)])
