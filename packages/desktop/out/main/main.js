...
var require_remove = __commonJS({
  "node_modules/electron-updater/node_modules/fs-extra/lib/remove/index.js"(exports2, module2) {
    ...
    function remove(path3, callback) {
      if (fs3.rm) return fs3.rm(path3, { recursive: true, force: true }, callback);
      rimraf(path3, callback);
    }
    function removeSync(path3) {
      if (fs3.rmSync) return fs3.rmSync(path3, { recursive: true, force: true });
      rimraf.sync(path3);
    }
    module2.exports = {
      remove: u(remove),
...
`));
      ...
      static reconstructOriginalUrl(options) {
        ...
        return new url_1.URL(`${protocol2}//${hostname}${port}${path3}`);
      }
  ...
  "node_modules/electron-updater/out/DownloadedUpdateHelper.js"(exports2) {
    ...
    var DownloadedUpdateHelper = class {
      ...
      get cacheDirForPendingUpdate() {
        return path3.join(this.cacheDir, "pending");
      }
      ...
      async getValidCachedUpdateFile(fileInfo, logger) {
        ...
        const updateFile = path3.join(this.cacheDirForPendingUpdate, cachedInfo.fileName);
      ...
      getUpdateInfoFile() {
        return path3.join(this.cacheDirForPendingUpdate, "update-info.json");
      }
    };
    ...
    async function createTempUpdateFile(name, cacheDir, log) {
      let nameCounter = 0;
      let result = path3.join(cacheDir, name);
      for (let i = 0; i < 3; i++) {
        try {
          ...
          result = path3.join(cacheDir, `${nameCounter++}-${name}`);
        }
      }
      return result;
    }
...
var require_AppAdapter = __commonJS({
  "node_modules/electron-updater/out/AppAdapter.js"(exports2) {
    ...
    function getAppCacheDir() {
      ...
      let result;
      if (process.platform === "win32") {
        result = process.env["LOCALAPPDATA"] || path3.join(homedir, "AppData", "Local");
      } else if (process.platform === "darwin") {
        result = path3.join(homedir, "Library", "Caches");
      } else {
        result = process.env["XDG_CACHE_HOME"] || path3.join(homedir, ".cache");
      }
      return result;
    }
  }
  ...
  "node_modules/electron-updater/out/ElectronAppAdapter.js"(exports2) {
    ...
    var ElectronAppAdapter = class {
      ...
      get appUpdateConfigPath() {
        return this.isPackaged ? path3.join(process.resourcesPath, "app-update.yml") : path3.join(this.app.getAppPath(), "dev-app-update.yml");
      }
      get userDataPath() {
    ...
    var PrivateGitHubProvider = class extends GitHubProvider_1.BaseGitHubProvider {
      ...
      resolveFiles(updateInfo) {
        return (0, Provider_1.getFileList)(updateInfo).map((it) => {
          const name = path3.posix.basename(it.url).replace(/ /g, "-");
  ...
  "node_modules/electron-updater/out/AppUpdater.js"(exports2) {
    ...
    var AppUpdater = class _AppUpdater extends events_1.EventEmitter {
      ...
      async getOrCreateStagingUserId() {
        const file = path3.join(this.app.userDataPath, ".updaterId");
        try {
      ...
      async getOrCreateDownloadHelper() {
        ...
        if (result == null) {
          ...
          const cacheDir = path3.join(this.app.baseCachePath, dirName || this.app.name);
          if (logger.debug != null) {
      ...
      async executeDownload(taskOptions) {
        ...
        let updateFile = path3.join(cacheDir, updateFileName);
        const packageFile = packageInfo == null ? null : path3.join(cacheDir, `package-${version}${path3.extname(packageInfo.path) || ".7z"}`);
        const done = async (isSaveCache) => {
          ...
          const currentBlockMapFile = path3.join(cacheDir, "current.blockmap");
          if (await (0, fs_extra_1.pathExists)(currentBlockMapFile)) {
            await (0, fs_extra_1.copyFile)(currentBlockMapFile, path3.join(downloadedUpdateHelper.cacheDir, "current.blockmap"));
          }
      ...
      async differentialDownloadInstaller(fileInfo, downloadUpdateOptions, installerPath, provider, oldInstallerFileName) {
        try {
          ...
          const downloadOptions = {
            newUrl: fileInfo.url,
            oldFile: path3.join(this.downloadedUpdateHelper.cacheDir, oldInstallerFileName),
            logger: this._logger,
          ...
          const saveBlockMapToCacheDir = async (blockMapData, cacheDir) => {
            const blockMapFile = path3.join(cacheDir, "current.blockmap");
  ...
  "node_modules/electron-updater/out/AppImageUpdater.js"(exports2) {
    ...
    var AppImageUpdater = class extends BaseUpdater_1.BaseUpdater {
      ...
      doInstall(options) {
        ...
        let destination;
        const existingBaseName = path3.basename(appImageFile);
        const installerPath = this.installerPath;
        if (installerPath == null) {
          this.dispatchError(new Error("No update filepath provided, can't quit and install"));
          return false;
        }
        if (path3.basename(installerPath) === existingBaseName || !/\d+\.\d+\.\d+/.test(existingBaseName)) {
          destination = appImageFile;
        } else {
          destination = path3.join(path3.dirname(appImageFile), path3.basename(installerPath));
        }
      ...
      async doDownloadUpdate(downloadUpdateOptions) {
        ...
        return this.executeDownload({
          ...
          task: async (destinationFile, downloadOptions) => {
            const cachedUpdateFilePath = path3.join(this.downloadedUpdateHelper.cacheDir, CURRENT_MAC_APP_ZIP_FILE_NAME);
      ...
      return new Promise((resolve, reject) => {
        ...
        (0, child_process_1.execFile)(...preparePowerShellExec(`"Get-AuthenticodeSignature -LiteralPath '${tempUpdateFile}' | ConvertTo-Json -Compress"`, 20 * 1e3), (error, stdout, stderr) => {
          ...
          try {
            ...
            if (data.Status === 0) {
              try {
                const normlaizedUpdateFilePath = path3.normalize(data.Path);
                const normalizedTempUpdateFile = path3.normalize(unescapedTempUpdateFile);
    ...
    var NsisUpdater = class extends BaseUpdater_1.BaseUpdater {
      ...
      doInstall(options) {
        ...
        const callUsingElevation = () => {
          this.spawnLog(path3.join(process.resourcesPath, "elevate.exe"), [installerPath].concat(args)).catch((e) => this.dispatchError(e));
        };
      ...
      async differentialDownloadWebPackage(downloadUpdateOptions, packageInfo, packagePath, provider) {
        ...
        try {
          const downloadOptions = {
            ...
            oldFile: path3.join(this.downloadedUpdateHelper.cacheDir, builder_util_runtime_1.CURRENT_APP_PACKAGE_FILE_NAME),
            logger: this._logger,
            newFile: packagePath,
  ...
  "node_modules/electron-updater/out/main.js"(exports2) {
    ...
    function doLoadAutoUpdater() {
      if (process.platform === "win32") {
        ...
        try {
          const identity = path3.join(process.resourcesPath, "package-type");
...
var Platform = {
  getDataDir() {
    ...
    subdirs.forEach((sub) => {
      const subPath = import_node_path.default.join(dir, sub);
      if (!import_node_fs.default.existsSync(subPath)) {
        import_node_fs.default.mkdirSync(subPath, { recursive: true });
      }
    });
    return dir;
  },
  getPythonPath() {
    ...
    if (import_electron.app.isPackaged) {
      ...
      resolvedPath = import_node_path.default.join(
      ...
      const venvPath = import_node_path.default.join(__dirname, "../../../ml-engine/venv/bin/python3");
      resolvedPath = import_node_fs.default.existsSync(venvPath) ? venvPath : process.env.AXIOM_PYTHON_PATH ?? "python3";
    }
  ...
  getPythonLibPath() {
    ...
    const srcDir = import_node_path.default.join(__dirname, "../../../ml-engine/src");
    const appDir = import_node_path.default.join(srcDir, "app");
    const paths = [
      srcDir,
      appDir,
      import_node_path.default.join(appDir, "services"),
      import_node_path.default.join(appDir, "utils"),
      import_node_path.default.join(appDir, "core")
    ];
    const archPath = process.arch === "arm64" ? "arm64" : "x64";
    const packagesDir = import_electron.app.isPackaged ? import_node_path.default.join(process.resourcesPath, "python-runtime", "darwin", "packages-" + archPath) : paths.join(":");
...
function setupUpdater(window2) {
  ...
  import_electron_updater.autoUpdater.on("download-progress", (progressObj) => {
    window2.webContents.send("update:progress", progressObj.percent);
  });
  import_electron_updater.autoUpdater.on("update-downloaded", () => {
  });
  import_electron2.ipcMain.on("update:install", () => {
...
import_electron3.app.setName("AXIOM");
var mainWindow = null;
var tray = null;
var sidecar = null;
...
var restartCount = 0;
var MAX_RESTARTS = 5;
var SIDECAR_PORT = 18432;
var DEFAULT_RENDERER_URL = "http://localhost:5173";
var APP_INDEX_URL = "app://./index.html";
function getRendererRoot() {
  return import_electron3.app.isPackaged ? import_node_path2.default.join(process.resourcesPath, "renderer") : import_node_path2.default.join(__dirname, "../renderer");
}
function buildCsp(forDevServer) {
...
function registerAppProtocol() {
  import_electron3.protocol.handle("app", async (request) => {
    const rendererRoot = getRendererRoot();
    const requestUrl = new URL(request.url);
    const requestedPath = decodeURIComponent(requestUrl.pathname);
    const relativePath = requestedPath === "/" ? "index.html" : requestedPath.replace(/^\/+/, "");
    const candidatePath = import_node_path2.default.resolve(rendererRoot, relativePath);
    const normalizedRoot = import_node_path2.default.resolve(rendererRoot);
    if (!candidatePath.startsWith(normalizedRoot)) {
      return new Response("Forbidden", { status: 403 });
    }
    let filePath = candidatePath;
    if (!import_node_fs2.default.existsSync(filePath) || import_node_fs2.default.statSync(filePath).isDirectory()) {
      const shouldServeIndex = import_node_path2.default.extname(relativePath) === "";
      if (!shouldServeIndex) {
        return new Response("Not Found", { status: 404 });
      }
      filePath = import_node_path2.default.join(normalizedRoot, "index.html");
    }
    return import_electron3.net.fetch((0, import_node_url.pathToFileURL)(filePath).toString());
  });
}
function installCsp() {
  import_electron3.session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    ...
    const isDevRequest = details.url.startsWith("http://localhost:5173") || details.url.startsWith("http://127.0.0.1:5173");
...
function canReachRenderer(urlString, timeoutMs = 1500) {
  return new Promise((resolve) => {
    try {
...
function spawnSidecar() {
  return new Promise((resolve, reject) => {
    ...
    const script = import_electron3.app.isPackaged ? import_node_path2.default.join(process.resourcesPath, "ml-engine", "src", "main.py") : import_node_path2.default.join(__dirname, "../../../ml-engine/src/main.py");
    console.log(`[Sidecar] Spawning: ${python} ${script} --port ${SIDECAR_PORT}`);
    console.log(`[Sidecar] PYTHONPATH: ${Platform.getPythonLibPath()}`);
    sidecar = (0, import_node_child_process.spawn)(python, [
      script,
      "--port",
      String(SIDECAR_PORT),
      "--data-dir",
    ...
    sidecar.on("exit", (code, signal) => {
      ...
      if (restartCount < MAX_RESTARTS && !import_electron3.app.isQuitting) {
        restartCount++;
...
async function createWindow() {
  mainWindow = new import_electron3.BrowserWindow({
    ...
    webPreferences: {
      ...
      preload: import_node_path2.default.join(__dirname, "../preload/preload.js")
    }
  });
...
function createTray() {
  const icon = import_electron3.nativeImage.createFromPath(
    import_node_path2.default.join(__dirname, "../resources/icons/tray.png")
  );
  tray = new import_electron3.Tray(icon.resize({ width: 16, height: 16 }));
...
function updateTray(status) {
  ...
  const menu = import_electron3.Menu.buildFromTemplate([
    {
      label: `AXIOM  \xB7  ${status.toUpperCase()}`,
      enabled: false
    },
    { type: "separator" },
    {
    ...
    { type: "separator" },
    {
      label: `Version ${import_electron3.app.getVersion()}`,
      enabled: false
    },
    {
      label: "Quit AXIOM",
      click: () => {
        import_electron3.app.isQuitting = true;
...
import_electron3.ipcMain.handle("app:get-platform", () => ({
  os: process.platform,
  arch: process.arch,
  version: import_electron3.app.getVersion(),
  dataDir: Platform.getDataDir()
}));
import_electron3.ipcMain.handle("app:open-external", (_, url) => {
  if (url.startsWith("https://")) {
    import_electron3.shell.openExternal(url);
  }
});
import_electron3.ipcMain.handle("app:open-data-dir", () => {
  import_electron3.shell.openPath(Platform.getDataDir());
});
import_electron3.ipcMain.handle("app:restart-sidecar", () => {
  sidecar?.kill("SIGTERM");
  sidecarReady = false;
  setTimeout(spawnSidecar, 2e3);
});
...
function hadPreviousSession() {
  ...
  return existsSync(
    import_node_path2.default.join(Platform.getDataDir(), "data", "user.sqlite")
  );
}
...
import_electron3.app.on("will-quit", (event) => {
...
import_electron3.app.on("web-contents-created", (_, contents) => {
...
import_electron3.app.isQuitting = false;
